...
Wayfind has three phases: **capture**, **index**, and **deliver**. Data enters as plain markdown files written during AI coding sessions and pulled from external tools. It gets indexed into a searchable content store. It leaves as persona-targeted digests and bot responses.
...
`wayfind reindex` builds a unified content store from all three sources. It runs three indexers sequentially:
...
All three write to the same content store — a single unified index regardless of source. Each entry carries a `source` field (`journal`, `conversation`, or `signal`) so the query layer can distinguish them when needed. The backend is SQLite when `better-sqlite3` is available, with JSON file fallback.

**Embeddings** are generated if `OPENAI_API_KEY` or `AZURE_OPENAI_EMBEDDING_*` vars are set. Stored alongside the index. Backfilled on re-index for entries that were previously indexed without them.
...
The Slack bot (`wayfind bot`) receives @mentions via Socket Mode:
...
`wayfind search-journals <query>` and `wayfind insights` read directly from the content store. Full-text search works with zero API keys. Semantic search requires an embedding API key.
...
Even solo engineers may work across multiple teams (e.g., a freelancer with repos bound to different clients). Each repo's `.claude/wayfind.json` determines which team context receives its journals.

```mermaid
flowchart LR
    Journals["~/.claude/memory/journal/"] --> Reindex["wayfind reindex"]
    Signals["team-context/signals/"] --> Reindex
    Reindex --> Store["Content store"]
    Store --> Digest["wayfind digest"]
```
...
| ③ | **Index** | Journal entries inside container | Container is team-scoped — only indexes its own team's journals and signals | Content store reflects one team's data | N/A — container boundary enforces isolation. |
...
**Unified index, source-agnostic.** Journals, conversations, and signals all live in one index. A query about "what's happening with billing?" hits support conversations, engineering decisions, and GitHub issues in a single search. The complexity of multiple data sources is absorbed at index time, not query time.

**Incremental everything.** Content hashing at every stage means re-running any operation is cheap. The container cron typically completes in seconds because nothing changed. This makes the system reliable without being expensive.
