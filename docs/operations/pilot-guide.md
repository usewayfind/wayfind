...
The bot is a thin LLM relay — it hands the user's question to Claude (Haiku) along with the same tools MCP clients use (`search_context`, `get_entry`). Claude decides how to search: what queries to run, what date ranges to filter, which entries to read in full. The bot posts Claude's synthesized answer as a thread reply. It acknowledges with an eyes emoji immediately so the team knows it's working.
...
**Container search API (v2.0.45+):** The container also exposes a search API (`POST /api/search`, `GET /api/entry/:id`) authenticated via a shared API key. The search API supports filtering by `until`, `user`, `source`, and `mode=browse` (list recent entries without a query). Team members' local MCP servers (`wayfind-mcp`) proxy semantic search to the container automatically — no Docker required on their end. This means the whole team gets semantic search over the full decision trail without each person running a container.
