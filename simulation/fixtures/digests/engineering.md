...
## Technical Debt
- EventSubscriptionService: 3 hardcoded config values bypassing config-as-code pattern (PR #2465). Quick fix shipped but underlying config model needs cleanup.
- AnalyticsService Cosmos-to-SQL migration left 22 mirror views that should be dropped after validation period.
...
## CI/CD
- 2 workflow failures on main this period (AnalyticsService). Both related to SQL MI connection timeout.
- EventSubscriptionService: 14 runs, 0 failures. Clean.

## Code Quality
- PR #2471 merged without tests (ImportExportService batch endpoint). Coverage dropped 3%.
